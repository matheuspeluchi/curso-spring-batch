...
public class FixedWidthFileReaderConfig {
  ...
  // @Bean
  @StepScope
  public FlatFileItemReader<ClientWithTransaction> fixedWidthFileReader(
      @Value("#{jobParameters['clientFile']}") Resource clientFile) {

    return new FlatFileItemReaderBuilder<ClientWithTransaction>().name("fixedWidthFileReader")
        .resource(clientFile)
        .fixedLength()
        .columns(new Range(1, 10), new Range(11, 20), new Range(21, 23), new Range(24, 43))
        .names("name", "lastName", "age", "email").targetType(ClientWithTransaction.class).build();
  }
}
